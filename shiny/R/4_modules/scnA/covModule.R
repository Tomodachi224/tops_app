...
covImpact<- function(input, output, session, toptab= "A",
                                              caption_text,
                                              param,
                                              deciledata,
                                              age5yrsdata,
  ...
  output$text_errors<- renderText({
    text <- exclusionerrors()[[paste0(toptab)]]
    return(paste0(text))
  })
  ...
  output$text_graphcov_exp <- renderText({
    ...
    text <-
      paste0(
        ...
        round(totaldata()[[paste0("meancov_", toptab)]][1]),
        ...
        round(data[data$decile==1 & data$female_head==2 & data$rural==2,  c(paste0("meancov_", toptab))]),
        " per cent in the lowest decile and ",
        round(data[data$decile==10 & data$female_head==2 & data$rural==2,  c(paste0("meancov_", toptab))]),
  ...
  plot_graphcov_exp <- reactive({
    if (intended_category()==1) {
      if (input$graphcov_exp_urb==FALSE) {
        plot <- plotGraphcov_exp(dataGraphcov_exp(d = deciledata(), d2 = totaldata(), t = c(toptab), int = c("int")))
      } else {
        plot <- plotGraphcov_exp_urb(dataGraphcov_exp_urb(d = deciledata(), d2 = totaldata(), t = c(toptab), int = c("int")))
      }
    } else {
      if (input$graphcov_exp_urb==FALSE) {
        plot <- plotGraphcov_exp(dataGraphcov_exp(d = deciledata(), d2 = totaldata(), t = c(toptab)))
      } else {
        plot <- plotGraphcov_exp_urb(dataGraphcov_exp_urb(d = deciledata(), d2 = totaldata(), t = c(toptab)))
      }
    }
      return(plot)
    }) 
  ...
  output$datavalues <- DT::renderDataTable({
    if (intended_category()==1) {
      data1 <- dataGraphcov_exp_urb(d = deciledata(), d2 = totaldata(), t = c(toptab), int = c("int"))
      data2 <- dataGraphcov_exp(d = deciledata(), d2 = totaldata(), t = c(toptab), int = c("int"))
    } else {
      data1 <- dataGraphcov_exp_urb(d = deciledata(), d2 = totaldata(), t = c(toptab))
      data2 <- dataGraphcov_exp(d = deciledata(), d2 = totaldata(), t = c(toptab))
    }
  ...
  output$text_graphcov_hhtype <- renderText({
    ...
    text <-
      paste0(
        ...
        round(data[data$hhtype3=="Couple household, with children" & data$female_head==2 & data$rural==2,  c(paste0("meancov_", toptab))]),
        " per cent of couple households with children would be covered, while ",
        round(data[data$hhtype3=="Couple household, with no children" & data$female_head==2 & data$rural==2,  c(paste0("meancov_", toptab))]),
        " per cent of couple households with no children are covered. About ",
        round(data[data$hhtype3=="One-person household, 60+ years" & data$female_head==2 & data$rural==2,  c(paste0("meancov_", toptab))]),
  ...
  plot_graphcov_hhtype <- reactive({
    if (input$graphcov_hhtype_urb==FALSE) {
      plot <- plotGraphcov_hht(dataGraphcov_hht(d = hhtypedata(), d2 = totaldata(), t = c(toptab)))
    } else {
      plot <- plotGraphcov_hht_urb(dataGraphcov_hht_urb(d = hhtypedata(), d2 = totaldata(), t = c(toptab)))
    }
    return(plot)
  }) 
  ...
  output$datavalues_hhtype <- DT::renderDataTable({
    data1 <- dataGraphcov_hht_urb(d = hhtypedata(), d2 = totaldata(), t = c(toptab))
    data2 <- dataGraphcov_hht(d = hhtypedata(), d2 = totaldata(), t = c(toptab))
