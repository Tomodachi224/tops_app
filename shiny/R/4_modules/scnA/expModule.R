...
expImpactUI<- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      condition = "input.country != 'Turkey'",
      popify(
...
expImpact<- function(input, output, session, toptab= "A",
                      caption_text,
                      param,
                      deciledata,
                      age5yrsdata,
  ...
  # observe({
  #   if (param$country=="Philippines" || param$country=="Maldives") {
  ...
  output$text_graphexp_all <- renderText({
    ...
    text <-
      paste0(
        ...
        round(totaldata()[[paste0("meanexp_", toptab)]][1]),
        " per cent on average. Across consumption expenditure deciles, increases will range from ",
        round(data[data$decile==1 & data$female_head==2 & data$rural==2,  c(paste0("meanexp_", toptab))]),
        " per cent in the lowest consumption expenditure decile, to ",
        round(data[data$decile==10 & data$female_head==2 & data$rural==2,  c(paste0("meanexp_", toptab))]),
  ...
  plot_graphexp_all <- reactive({
    if (intended_category()==1) {
      if (input$graphexp_all_urb==FALSE) {
        plot <- plotGraphexp_all(deciledata(), totaldata(), t = c(toptab), int = c("int"))
      } else {
        plot <- plotGraphexp_all_urb(deciledata(), totaldata(), t = c(toptab), int = c("int"))
      }
    } else {
      if (input$graphexp_all_urb==FALSE) {
        plot <- plotGraphexp_all(deciledata(), totaldata(), t = c(toptab))
      } else {
        plot <- plotGraphexp_all_urb(deciledata(), totaldata(), t = c(toptab))
      }
    }
    return(plot)
  }) 
  ...
  output$text_graphexp_ben <- renderText({
    ...
    text <-
      paste0(
        ...
        round(totaldata()[[paste0("meanbenexp_", toptab)]][1]),
        " per cent, on average. Across consumption expenditure deciles, increases will range from ",
        round(data[data$decile==1 & data$female_head==2 & data$rural==2,  c(paste0("meanbenexp_", toptab))]),
        " per cent in the lowest consumption expenditure decile, to  ",
        round(data[data$decile==10 & data$female_head==2 & data$rural==2,  c(paste0("meanbenexp_", toptab))]),
        " per cent in the highest consumption expenditure decile."
      )
    return(text)
  })
  plot_graphexp_ben <- reactive({
    if (input$graphexp_ben_urb==FALSE) {
      plot <- plotGraphexp_ben(deciledata(), totaldata(),  t = c(toptab))
    } else {
      plot <- plotGraphexp_ben_urb(deciledata(), totaldata(), t = c(toptab))
    }
    return(plot)
  }) 
  ...
  output$text_graphexp_hhtype <- renderText({
    ...
    text <-
      paste0(
        ...
        round(data[data$hhtype3=="Couple household, with children" & data$female_head==2 & data$rural==2,  c(paste0("meanbenexp_", toptab))]),
        " per cent for couple households with children, ",
        round(data[data$hhtype3=="Couple household, with no children" & data$female_head==2 & data$rural==2,  c(paste0("meanbenexp_", toptab))]),
        " per cent for couple households with no children, and ",
        round(data[data$hhtype3=="One-person household, 60+ years" & data$female_head==2 & data$rural==2,  c(paste0("meanbenexp_", toptab))]),
  ...
  plot_graphexp_hhtype <- reactive({
    if (input$graphexp_hhtype_urb==FALSE) {
      plot <- plotGraphexp_hht(hhtypedata(), totaldata(),  t = c(toptab))
    } else {
      plot <- plotGraphexp_hht_urb(hhtypedata(), totaldata(), t = c(toptab))
    }
    return(plot)
  }) 
  ...
  output$datavalues_all <- DT::renderDataTable({
    ...
    if (param$country=="Philippines" || param$country=="Maldives" || param$country=="Turkey") {
      data <- data.frame(
        Deciles = c("Lowest", "2", "3", "4", "5", "6", "7", "8", "9", "Highest", "Total"),
        Total = round(c(data_total[[paste0(varname,toptab)]], total[total$rural==2, c(paste0(varname, toptab))]),1)
      )
      target <- c(1)
    } else {
      data <- data.frame(
        Deciles = c("Lowest", "2", "3", "4", "5", "6", "7", "8", "9", "Highest", "Total"),
        Total = round(c(data_total[[paste0(varname,toptab)]], total[total$rural==2, c(paste0(varname, toptab))]),1),
        Urban = round(c(data_urban[[paste0(varname,toptab)]], total[total$rural==0, c(paste0(varname, toptab))]),1),
        Rural = round(c(data_rural[[paste0(varname,toptab)]], total[total$rural==1, c(paste0(varname, toptab))]),1)
      )
      target <-c(1:3)
    }
  ...
  output$datavalues_ben <- DT::renderDataTable({
    ...
    if (param$country=="Philippines" || param$country=="Maldives" || param$country=="Turkey") {
      data <- data.frame(
        Deciles = c("Lowest", "2", "3", "4", "5", "6", "7", "8", "9", "Highest", "Total"),
        Total = round(c(data_total[[paste0("meanbenexp_",toptab)]], total[total$rural==2, c(paste0("meanbenexp_", toptab))]),1)
      )
      target <- c(1)
    } else {
      data <- data.frame(
        Deciles = c("Lowest", "2", "3", "4", "5", "6", "7", "8", "9", "Highest", "Total"),
        Total = round(c(data_total[[paste0("meanbenexp_",toptab)]], total[total$rural==2, c(paste0("meanbenexp_", toptab))]),1),
        Urban = round(c(data_urban[[paste0("meanbenexp_",toptab)]], total[total$rural==0, c(paste0("meanbenexp_", toptab))]),1),
        Rural = round(c(data_rural[[paste0("meanbenexp_",toptab)]], total[total$rural==1, c(paste0("meanbenexp_", toptab))]),1)
      )
      target <-c(1:3)
    }
