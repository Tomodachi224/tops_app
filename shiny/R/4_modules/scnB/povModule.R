...
povImpact_b <- function(input, output, session, toptab_b = "A",
                      impact_data_b, 
                      caption_text_b,
                      age5yrsdata_b) {
  ...
  data_for_textpovineq <- reactive({
    ...
    after1 <-
      weighted.mean(data[[paste0("post_pov_", toptab_b)]], data$weight)
    reduction1 <- (before1 - after1) * 100 / before1
    
    before2 <-
      weighted.mean(data$gap, data$weight)
    after2 <-
      weighted.mean(data[[paste0("post_gap_", toptab_b)]], data$weight)
    reduction2 <- (before2 - after2) * 100 / before2
    
    check <- sum(data[[paste0("cov_", toptab_b)]])
    
    if (check>0) {
      data$after <-
        data$adj_welfare - data[[paste0("value_", toptab_b)]] + data[[paste0("transfer_", toptab_b)]]
    } else {
      data$after <-
        data$adj_welfare
    }
    ...
    is.na(data[[paste0("transfer_", toptab_b)]]) <-
      data[[paste0("transfer_", toptab_b)]] == 0
    data <- data[!is.na(data[[paste0("transfer_", toptab_b)]]),]
  ...
  datapov <- reactive({
    ...
    current1 <-
      weighted.mean(data$pov, data$weight)
    before1 <-
      weighted.mean(data[[paste0("pre_pov_", toptab_b)]], data$weight)
    after1 <-
      weighted.mean(data[[paste0("post_pov_", toptab_b)]], data$weight)
    ...
    before2 <-
      weighted.mean(data[[paste0("pre_povI1_", toptab_b)]], data$weight)
    after2 <-
      weighted.mean(data[[paste0("post_povI1_", toptab_b)]], data$weight)
    reduction2 <- (current2 - after2) * 100 / current2
    
    current3 <-
      weighted.mean(data$povI2, data$weight)
    before3 <-
      weighted.mean(data[[paste0("pre_povI2_", toptab_b)]], data$weight)
    after3 <-
      weighted.mean(data[[paste0("post_povI2_", toptab_b)]], data$weight)
    reduction3 <- (current3 - after3) * 100 / current3
    
    current4 <-
      weighted.mean(data$povI3, data$weight)
    before4 <-
      weighted.mean(data[[paste0("pre_povI3_", toptab_b)]], data$weight)
    after4 <-
      weighted.mean(data[[paste0("post_povI3_", toptab_b)]], data$weight)
    ...
    check1 <- sum(impact_data_b()[[paste0("value_", toptab_b)]])
    if (check1 > 0) {
  ...
  datagap <- reactive({
    ...
    current1 <-
      weighted.mean(data$gap, data$weight)
    before1 <-
      weighted.mean(data[[paste0("pre_gap_", toptab_b)]], data$weight)
    after1 <-
      weighted.mean(data[[paste0("post_gap_", toptab_b)]], data$weight)
    reduction1 <- (current1 - after1) * 100 / current1
    ...
    before2 <-
      weighted.mean(data[[paste0("pre_gapI1_", toptab_b)]], data$weight)
    after2 <-
      weighted.mean(data[[paste0("post_gapI1_", toptab_b)]], data$weight)
    reduction2 <- (current2 - after2) * 100 / current2
    
    current3 <-
      weighted.mean(data$gapI2, data$weight)
    before3 <-
      weighted.mean(data[[paste0("pre_gapI2_", toptab_b)]], data$weight)
    after3 <-
      weighted.mean(data[[paste0("post_gapI2_", toptab_b)]], data$weight)
    reduction3 <- (current3 - after3) * 100 / current3
    
    current4 <-
      weighted.mean(data$gapI3, data$weight)
    before4 <-
      weighted.mean(data[[paste0("pre_gapI3_", toptab_b)]], data$weight)
    after4 <-
      weighted.mean(data[[paste0("post_gapI3_", toptab_b)]], data$weight)
    reduction4 <- (current4 - after4) * 100 / current4
  ...
  output$tabgini <- renderTable({
    ...
    data$before <-
      data$adj_welfare - data[[paste0("value_", toptab_b)]]
